#!/usr/bin/env bash
# V5.1 overnight pipeline - Machine A / shard 0 of 2
#
# 用法：
#   1) 先 cd 到包含以下两个文件的目录：
...
OUTPUT_DIR="${OUTPUT_DIR:-layouts_260_v5_shard0}"
LOG_DIR="${LOG_DIR:-v5_overnight_logs_shard0}"
STATE_DIR="${STATE_DIR:-v5_overnight_state_shard0}"

# 本脚本固定负责 2-way 分片中的 shard 0。
# 两台机器分别运行 A/B 脚本即可，任务严格互斥、不会重复。
SHARD_COUNT=2
SHARD_INDEX=0
...
# ---------- preflight ----------
log "V5.1 overnight pipeline starting - MACHINE A / SHARD 0 OF 2"
log "cwd=$(pwd)"
...
log "最终 library：$OUTPUT_DIR/library_shard00of02.jsonl"
