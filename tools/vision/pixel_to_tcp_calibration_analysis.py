...
# 8-18 凌晨修正：只有 panel-180621 批次数据可用；194117/190632/174744/190832
# 是当晚因异常中途停止的批次，"成功"记录的 TCP 不可信，禁止混入拟合。
# 8-18 晨离群点剔除（用户确认）：180621 内去掉 6 条——任务12 L_yellow(4.43mm)、
# 任务22 L_blue(2.13)、任务25 line(1.34)、任务28 line(1.32)、任务16 L_yellow(1.21)、
# 任务30 T(1.13)；被剔点在去掉后自身误差不降，确认真离群。像素-tcp 拟合 29 条；
# 15 条确认点（不含任务12,16 之外仍含任务16——九点侧 poly2 拟合下残差正常）
# 仅供九点标定。
BLOCK_EXPECTED_SUCCESS_RANGE = (29, 29)
