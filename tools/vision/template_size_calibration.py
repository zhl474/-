...
PACKAGE_DIR = os.path.join(SRC_DIR, "image_process")
...
SHOW_WINDOW = True
AUTO_ACCEPT = False
TARGET_CATEGORIES = None  # 例如 ["T", "square"]；None 表示全部类别。
...
def parse_args():
    """把文件开头参数转换成原处理流程使用的配置对象。"""
    return SimpleNamespace(
        image=list(INPUT_IMAGES),
        camera=bool(USE_CAMERA or not INPUT_IMAGES),
        topic=CAMERA_TOPIC,
        frames=int(FRAME_COUNT),
        timeout=float(WAIT_TIMEOUT_SEC),
        detect_model=DETECT_MODEL_PATH,
        output=TEMPLATE_CONFIG_PATH,
        vis_dir=DEFAULT_VIS_DIR,
        save_vis=bool(SAVE_VISUALIZATION),
        no_window=not SHOW_WINDOW,
        auto_accept=bool(AUTO_ACCEPT),
        accumulate=False,
        category=TARGET_CATEGORIES,
        detect_conf=float(DETECT_CONFIDENCE),
        iou=float(DETECT_IOU),
        crop_margin=int(CROP_MARGIN_PX),
        mask_thresh=int(MASK_THRESHOLD),
        min_area=float(MIN_CONTOUR_AREA),
        calibration=CALIBRATION_MATRIX_PATH,
        no_undistort=False,
    )
