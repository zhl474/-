...
def _l_grab_point_v2(mask, angle, center, b, c, b_local=(0.0, 0.0)):
    ...
    沿长轴方向建立局部坐标系，对每个前景像素计算它到B格子理论中心的
    高斯权重 wB = exp(-dB²/(2σ²))，取加权质心作为grab point。

    参数:
        mask:    二值掩码
        angle:   L型长轴方向（弧度），A→B→C方向
        center:  (cx, cy) 旋转中心（画布中心 / minAreaRect 中心）
        b:       格子边长（像素）
        c:       缝隙宽度（像素）
        b_local: (pu_b, pv_b) B格子中心在局部坐标系中的偏移，
                 即 center→B 向量旋转到长轴坐标系后的 (u, v) 分量。
                 默认 (0,0) 表示 center 即 B 中心。
    """
    ...
    for angle in angles:
        ...
        if is_l:
            pick_canvas = _template_l_pick_in_canvas(
                binary, item["bbox"], item["anchor"], rect_size, angle, block_px, connector_px
            )
            pick = (
