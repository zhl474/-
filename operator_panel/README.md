...
“保存”只落盘并标记待重启；“保存并应用”只在空闲时重启控制台拥有的相应节点。外部节点永远不会被网页结束。
...
`competition.launch` 和 `calibration.launch` 都复用新的 `perception.launch`，并继续启动原交互脚本。独立 Mask 编辑窗口也保持不变。
...
V1 不提供网页 Mask 绘制、任意 TCP 位姿运动或点动、任务布局修改、标定矩阵逐项编辑、耗时/往复测试和局域网访问。
