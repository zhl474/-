...
“启动与模式”卡片还会只读检查两个 USB 设备：Orbbec Gemini 335 相机（`2bc5:0800`）和 HL-340 舵机串口（`1a86:7523`，`/dev/servo_motor`）。如果相机或舵机被本机其它项目脚本打开，点击“启动硬件”会立即显示占用 PID 和命令行并停止，不再等待硬件启动超时；如果 USB 设备被上位机独占或未接入本机，也会直接提示。该检查只读，不会结束任何外部进程。
...
- 结束 ArUco 单次对准等控制台启动的一次性工具子进程，避免继续发送运动命令；
...
手动控制页提供 `tools/vision/aruco_pixel_tcp_diagnostic/align_aruco_once.py` 的网页入口。它由控制台以独立 ROS 节点子进程运行，不需要启动感知模式，但要求：

- 控制服务就绪并持续收到新相机画面；
- 提供 `/camera/stable_world_points` 稳定深度服务；
- 页面已确认工作区安全。

网页只覆盖低位固定 TCP Z，并通过 `--yes` 跳过终端确认；ArUco 字典、标记 ID、检测帧数和伺服阈值仍以脚本开头参数区为准。脚本输出实时转发到“运行日志”，成功后读取最终实测 TCP；机械臂保持在最终对准位置，不会自动返回高位。点击“停止运动”会先结束该子进程再执行 `StopMotion()`。
